...
function forgot({navigation, route}) {
  ...
  const loginValidationSchema = yup.object().shape({
    Number: yup
      ...
      .test('Number', 'Invalid Number!', val =>
        isValidNumber(val, countrycode),
      ),
    // .test(isValidNumber)
  ...
  const formatPhoneNumber = val => {
    try {
      if (val.length !== 10) {
        ...
        const FormattedNumber = phoneUtil.formatInOriginalFormat(
          number,
          countrycode,
        );
  ...
  return (
    <View style={{flex: 1}}>
      <ScrollView scrollEnabled={true}>
        ...
        <View>
          <TouchableOpacity
            disabled={!formik.values.Number}
            style={!formik.values.Number ? styles.resetbtndis : styles.resetbtn}
